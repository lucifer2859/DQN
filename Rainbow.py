...
current_model = RainbowDQN(env.observation_space.shape[0], env.action_space.n, num_atoms, Vmin, Vmax).to(device)
target_model  = RainbowDQN(env.observation_space.shape[0], env.action_space.n, num_atoms, Vmin, Vmax).to(device)
...
def CartPole_plot(frame_idx, rewards, losses):
    ...
    plt.title('loss')
    plt.plot(losses)
    plt.savefig('img/Rainbow_CartPole_%s.png' % (frame_idx))
    plt.cla()
    plt.close("all")
...
for frame_idx in range(1, num_frames + 1):
    ...
    if frame_idx % 200 == 0:
        ...
        if frame_idx > 200:
            os.system('rm img/Rainbow_CartPole_%s.png' % (frame_idx - 200))
...
def Atari_plot(frame_idx, rewards, losses):
    ...
    plt.title('loss')
    plt.plot(losses)
    plt.savefig('img/Rainbow_Atari_%s.png' % (frame_idx))
    plt.cla()
    plt.close("all")
...
for frame_idx in range(1, num_frames + 1):
    ...
    if frame_idx % 10000 == 0:
        ...
        if frame_idx > 10000:
            os.system('rm img/Rainbow_Atari_%s.png' % (frame_idx - 10000))
